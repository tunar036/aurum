        ...
        <div class="d-flex flex-column-fluid">
            <div class="container">
                <div class="card card-custom example example-compact">
                    @include('backend.includes.form.header', ['page' => 'installment-cards'])
                    <form action="{{ $edit === false ? route('backend.installment-cards.store') : route('backend.installment-cards.update', ['installment_card' => $installment_card->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @if($edit)
                            @method('PUT')
                        @endif
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-6 col-md-9 col-sm-12">
                                                <div class="input-group">
                                                    <input id="name:{{ $lang->code }}" type="text" class="form-control @if($errors->has("name:$lang->code")) is-invalid @endif"  name="name:{{ $lang->code }}" value="{{ isset($installment_card) ? $installment_card->translate($lang->code)->name : old('name:'.$lang->code) }}" placeholder="@lang('backend.placeholders.enter.name')">
                                ...
                                <div class="col-lg-6 col-md-9 col-sm-12">
                                    <div class="input-group">
                                         <span class="switch switch-md switch-icon">
                                             <label>
                                                 <input type="checkbox" class="bool" name="status" value="{{ isset($installment_card) ? $installment_card->status : old('status') }}"  {{ (isset($installment_card) ? old('status',$installment_card->status) : old('status',1) ) == 1 ? 'checked' : '' }}>
                                                 <span></span>
                                             </label>
                                         </span>
                                    </div>
                    ...
                    @if ($edit)
                        @include('backend.includes.media',[
                            'model' => $installment_card,
                            'name'  => 'installment-cards',
                            'media_collection_name'  => 'installment_card_icon',
                            'isDeleted' => true,
                            'isCovered' => false,
                        ])
                    @endif
